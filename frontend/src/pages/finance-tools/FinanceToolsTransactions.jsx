...
function FinanceToolsTransactions() {
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      // Parse amount dengan benar: pastikan selalu number murni (bukan string yang sudah diformat)
      let amountValue = 0
      if (formData.amount) {
        if (typeof formData.amount === 'string') {
          // Jika string, parse dulu (handle jika sudah diformat dengan titik)
          amountValue = parseRupiahInput(formData.amount)
        } else if (typeof formData.amount === 'number') {
          // Jika sudah number, langsung pakai
          amountValue = formData.amount
        } else {
          // Fallback: parse float
          amountValue = parseFloat(formData.amount) || 0
        }
        // Pastikan integer (tidak ada decimal)
